...
if debug:
    ...
    for var in ['array_firm','array_num','array_num2']:
        exec("%s  = np.zeros((n_vis_data, n_vis_record))" % var)
...
while True:
    ...
    if sys.stdin in select.select([sys.stdin], [], [], 0)[0]:
        line = raw_input()
        break
    # stop while reaching the number of visualized records
    elif debug:
        if int(n_record) == n_vis_record:
            break
